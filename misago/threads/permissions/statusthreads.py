...
from misago.categories import STATUS_THREADS_ROOT_NAME
...
__all__ = [
    'allow_use_status_threads',
    'can_use_status_threads',
    'allow_see_status_thread',
    'can_see_status_thread',
    'allow_change_owner',
    'can_change_owner',
...
class PermissionsForm(forms.Form):
    legend = _("Status threads")

    can_use_status_threads = YesNoSwitch(label=_("Can use status threads"))
    can_start_status_threads = YesNoSwitch(label=_("Can start status threads"))
    max_status_thread_participants = forms.IntegerField(
        label=_("Max number of users invited to status thread"),
        help_text=_("Enter 0 to don't limit number of participants."),
        initial=3,
        min_value=0,
    )
    can_add_everyone_to_status_threads = YesNoSwitch(
        label=_("Can add everyone to threads"),
        help_text=_("Allows user to add users that are blocking him to status threads."),
    )
    can_report_status_threads = YesNoSwitch(
        label=_("Can report status threads"),
        help_text=_(
            "Allows user to report status threads they are "
            "participating in, making them accessible to moderators."
        ),
    )
    can_moderate_status_threads = YesNoSwitch(
        label=_("Can moderate status threads"),
        help_text=_(
            "Allows user to read, reply, edit and delete content "
            "in reported status threads."
        ),
    )
...
def build_acl(acl, roles, key_name):
    new_acl = {
        'can_use_status_threads': 0,
        'can_start_status_threads': 0,
        'max_status_thread_participants': 3,
        'can_add_everyone_to_status_threads': 0,
        'can_report_status_threads': 0,
    ...
    algebra.sum_acls(
        ...
        roles=roles,
        key=key_name,
        can_use_status_threads=algebra.greater,
        can_start_status_threads=algebra.greater,
        max_status_thread_participants=algebra.greater_or_zero,
        can_add_everyone_to_status_threads=algebra.greater,
        can_report_status_threads=algebra.greater,
        can_moderate_status_threads=algebra.greater
    )

    if not new_acl['can_use_status_threads']:
        new_acl['can_start_status_threads'] = 0
        return new_acl

    status_category = Category.objects.status_threads()

    new_acl['visible_categories'].append(status_category.pk)
    new_acl['browseable_categories'].append(status_category.pk)

    if new_acl['can_moderate_status_threads']:
        new_acl['can_see_reports'].append(status_category.pk)
    ...
    category_acl = {
        ...
        'can_start_threads': new_acl['can_start_status_threads'],
        'can_reply_threads': 1,
        'can_edit_threads': 1,
        'can_edit_posts': 1,
        ...
        'can_report_content': new_acl['can_report_status_threads'],
        'can_see_reports': 0,
    ...
    new_acl['categories'][status_category.pk] = category_acl

    return new_acl


def add_acl_to_thread(user, thread):
    if thread.thread_type.root_name != STATUS_THREADS_ROOT_NAME:
        return
...
def allow_use_status_threads(user):
    if user.is_anonymous:
        raise PermissionDenied(_("You have to sign in to use status threads."))
    if not user.acl_cache['can_use_status_threads']:
        raise PermissionDenied(_("You can't use status threads."))


can_use_status_threads = return_boolean(allow_use_status_threads)


def allow_see_status_thread(user, target):
    if user.acl_cache['can_moderate_status_threads']:
...
can_see_status_thread = return_boolean(allow_see_status_thread)


def allow_change_owner(user, target):
    is_moderator = user.acl_cache['can_moderate_status_threads']
...
def allow_add_participants(user, target):
    ...
    max_participants = user.acl_cache['max_status_thread_participants']
...
def allow_add_participant(user, target):
    ...
    if not can_use_status_threads(target):
        raise PermissionDenied(
            _("%(user)s can't participate in status threads.") % message_format
        )

    if user.acl_cache['can_add_everyone_to_status_threads']:
        return
    ...
    if target.can_be_messaged_by_nobody:
        raise PermissionDenied(
            _("%(user)s is not allowing invitations to status threads.") % message_format
        )

    if target.can_be_messaged_by_followed and not target.is_following(user):
        message = _("%(user)s limits invitations to status threads to followed users.")
